...
def main():
    ...
    logger.info('eval_it_pool: \n')
    for i in eval_it_pool:
        logger.info(i)
    ...
    for exp in range(args.num_exp):
        ...
        logger.info('Evaluation model pool: ')
        
        for i in model_eval_pool:
            logger.info(i)
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                for model_eval in model_eval_pool:
                    ...
                    wandb.log({f"Mean acc {str(model_eval)}":np.mean(accs), f"Std acc {str(model_eval)}":np.std(accs), f"Iteration {str(model_eval)}":it})
            ...
            embed = net.net.module.embed if torch.cuda.device_count() > 1 else net.net.embed # for GPU parallel
